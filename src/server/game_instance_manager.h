...
public:
    ...
    static bool get_game_instance(const std::string& game_id, GameInstance*& game_instance_ptr);
    ...
    static bool add_player_to_any_game(Player* player, GameInstance*& game_instance_ptr, std::string& err);

    // Try to add 'player' to the provided 'game_instance_ptr'. Returns true if success and false otherwise.
    static bool add_player(Player* player, GameInstance*& game_instance_ptr, std::string& err);
