...
is_sunk: bool, defines if the ship has sunk (TRUE) or not (FALSE), this is the
case when ship_coord is empty damage: short, holds remaining length of the
ship, gets updated by shot_at

The class operations are:
shot_at, void, updates damage and checks is_sunk, then sets the value
accordingly.
*/

// // Constructor
// Ship::Ship(unsigned int length_, ShipCategory name_) : length(length_),
// name(name_), is_sunk(false) {
//     damage = length;
// }
...
Ship::Ship(ShipCategory name_) : name(name_), is_sunk(false) {
    ...
    // if(length == 0) throw std::runtime_error(std::string("Not a valid
    // ShipCategory"));
    damage = length;
}
